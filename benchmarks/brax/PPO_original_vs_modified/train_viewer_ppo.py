...
from config import HOST, BASE_PORT, XML_MODEL, HYPERPARAMS, COMMON_PARAMS, NUM_BENCHMARK_RUNS
...
def main():
    ...
    json_path = f'benchmarks/brax/PPO_original_vs_modified/viewer_ppo_results_{hardware_name}.json'
    ...
    for param_name, params in HYPERPARAMS.items():
        print(f"\n--- Testing with '{param_name}' parameters (Viewer PPO) ---")

        viewer = WebViewer(host=HOST, port=BASE_PORT, xml=XML_MODEL, num_envs=params['num_envs'])
        viewer.run()
        
        sender = BraxSender(host=HOST, port=BASE_PORT, xml=XML_MODEL, num_envs=params['num_envs'])
        sender.start()
